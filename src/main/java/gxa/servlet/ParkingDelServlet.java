...
@WebServlet("/page/parking/delete")
public class ParkingDelServlet extends HttpServlet {
    @Override
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        if (ids.size()==0){
            ...
            parkingService.delete(Integer.parseInt(id));
        } else if (ids.size()!=0){
            for(int i=0;i<ids.size();i++){
                ParkingService parkingService=new ParkingServiceImpl();
                parkingService.delete(Integer.parseInt(ids.get(i)));
            }
        }
